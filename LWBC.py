...
def train_epoch(auxiliary_classifiers, main_classifier, data_loader, subsets, kl_loss, loss_fn, optimizer, optimizer_target, device, scheduler, scheduler_target, cur_threshold, alpha, temperature, lamda):

    auxiliary_classifiers = [classifier.train() for classifier in auxiliary_classifiers]
    main_classifier = main_classifier.train()
    losses = []
    i = 0
    pred_list = []
    true_list = []
    ...
    for d in data_loader:
        ...
        outputs = main_classifier(input_ids=input_ids, attention_mask=attention_mask)
        commit_outputs = [classifier(input_ids=input_ids, attention_mask=attention_mask) for classifier in auxiliary_classifiers]
        commit_preds = [(output > cur_threshold).to(torch.float) for output in commit_outputs]
        commit_acc = [(commit_pred == categories).to(torch.int) for commit_pred in commit_preds]
        commit_sum = commit_acc[0]
        for i in range(1, len(commit_acc)):
            commit_sum += commit_acc[i]
        loss_weight = (1 / (commit_sum / len(auxiliary_classifiers) + alpha)).detach()
        breakpoint()


        # _, preds = torch.max(outputs, dim=1)
        loss_wce = loss_fn(outputs, categories * loss_weight)
        preds = (outputs > cur_threshold).to(torch.float)

        
        losses.append(loss_wce.item())
        pred_list.append(preds.tolist())
        true_list.append(categories.tolist())
            
        loss_wce.backward()
        nn.utils.clip_grad_norm_(main_classifier.parameters(), max_norm=1.0)
        optimizer_target.step()
        scheduler_target.step()
        optimizer_target.zero_grad()

        for classifier_index in range(len(auxiliary_classifiers)):
            classifier = auxiliary_classifiers[classifier_index]

            # get the complement set of subset
            subset = subsets[classifier_index]
            subset_texts = [sample['text'] for sample in subset]
            input_idss = []
            attention_masks = []
            categoriess = []
            for sample_index in range(len(d['text'])):
                if d['text'][sample_index] in subset_texts: continue
                input_idss.append(d['input_ids'][sample_index])
                attention_masks.append(d['attention_mask'][sample_index])
                categoriess.append(d['categories'][sample_index])
            
            opts = classifier(input_ids=input_idss, attention_mask=attention_masks)
            if classifier_index == 0:
                # last version modify to here
...
def warm_up(classifiers, data_loader, subsets, epochs, loss_fn, device):

    params = []
    for classifier_index in range(len(classifiers)):
        classifiers[classifier_index] = classifiers[classifier_index].train()
        params += list(classifiers[classifier_index].parameters())
    optimizer=torch.optim.Adam(params, lr=2e-5, weight_decay=0)
    ...
    for epoch in range(epochs):
        for d in tqdm(data_loader, desc="Warm up epoch " + str(epoch)):
            input_ids = d["input_ids"].to(device)
            attention_mask = d["attention_mask"].to(device)
            categories = d["categories"].to(device)
            for classifier_index in range(len(classifiers)):
                ...
                for sub_d in subset:
                    input_idss.append(torch.unsqueeze(sub_d["input_ids"], dim=0).to(device))
                    attention_masks.append(torch.unsqueeze(sub_d["attention_mask"], dim=0).to(device))
                    categoriess.append(torch.unsqueeze(sub_d["categories"], dim=0).to(device))
                input_idss = torch.cat(input_idss, dim=0)
                attention_masks = torch.cat(attention_masks, dim=0)
                categoriess = torch.cat(categoriess, dim=0)
                outputs = classifier(input_ids=input_idss, attention_mask=attention_masks)
                if classifier_index == 0:
                    loss = loss_fn(outputs, categoriess)
                else:
                    loss += loss_fn(outputs, categoriess)
            
                preds = (outputs > cur_threshold).to(torch.float)
                pred_list.append(preds.tolist())
                true_list.append(categoriess.tolist())
...
def train_model(train_set, val_set, pretrained_model = 'bert-base-cased', 
                ...
                num_classifier = 5, warmup_epochs = 20, subsetsize = 0.01,
                alpha = 0.02, temperature = 1.0, lamda = 0.6):
    ...
    auxiliary_classifiers = [CategoryClassifier(len(label_list), bert_model).to(device) for _ in range(num_classifier)]
    main_classifier = CategoryClassifier(len(label_list), bert_model).to(device)

    # bootstrap for warm up
    subsets = []
    for i in range(num_classifier):
        subset = []
        random_indices = np.random.choice(len(train_set), int(len(train_set) * subsetsize), replace=False)
        for index in random_indices:
            subset.append(train_set[index])
        ...
        subset = PreparedDataset(texts=np.array(texts),
                                categories=np.array(categories),
                                tokenizer=tokenizer,
                                max_len=max_len)
        subsets.append(subset)

    warm_up(auxiliary_classifiers, train_data_loader, subsets, warmup_epochs, nn.CrossEntropyLoss().to(device), device)

    ## SINGLE optimizer
    params = []
    for i in range(num_classifier):
        params += list(auxiliary_classifiers[i].parameters())
    optimizer=torch.optim.Adam(params, lr=2e-5, weight_decay=0)
    optimizer_target=torch.optim.Adam(main_classifier.parameters(), lr=2e-5, weight_decay=0)
    ...
    scheduler_target = get_linear_schedule_with_warmup(
        optimizer_target,
        num_warmup_steps=0,
        num_training_steps=total_steps
        )
    ...
    for epoch in range(epochs):
        ...
        train_result, train_loss = train_epoch(auxiliary_classifiers, main_classifier, train_data_loader, subsets, kl_loss, loss_fn, optimizer, optimizer_target, device, scheduler, scheduler_target, cur_threshold, alpha, temperature, lamda)
...
def test_dataset(test_set, 
    ...
    bert_model = BertModel.from_pretrained(pretrained_model)
    model = CategoryClassifier(len(label_list), bert_model)
...
def main(args):
    ...
    if (args.mode == 'train'):
        run = wandb.init(
            ...
            project= "BERT with LBC on " + ("BDISen" if "BDISen" in args.test_path else "DepressionEmo"),
            name = current_time,
        ...
        train_model(train_set, val_set, pretrained_model = args.model_name, max_len = args.max_length, batch_size = args.batch_size,
                     ...
                     num_classifier = args.num_classifier, warmup_epochs = args.warmup_epochs, subsetsize = args.subsetsize,
                     alpha = args.alpha, temperature = args.temperature, lamda = args.kd_lambda)
        wandb.finish()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--subsetsize', type=float, default=0.01)
    ...
    parser.add_argument('--resume_path', type=str, default='bert-base-LWBC/'+current_time)
