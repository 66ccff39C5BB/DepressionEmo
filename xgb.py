...
savedir = os.path.join('xgb', current_time)
os.makedirs(savedir)
...
# learning_rate = 0.01
models = [xgb.XGBClassifier(max_depth=8, n_estimators=100, learning_rate=0.5) for i in range(len(label_list))]
for i in trange(len(label_list), desc='Training'):
    models[i].fit(x_train,y_train[i])
    pickle.dump(models[i], open(savedir+f"/model_{i}.pkl", "wb"))
